...
def first_pass( commands ):
    ...
    if basename == "" and len(vary) > 0:
        basename = "none"
        print "No basename was given"
    if nframes == 1 and len(vary) > 0:
        quit()
    
    return [nframes, basename]
...
def second_pass( commands, num_frames ):
    ...
    for y in range(num_frames):    
        for cmd in commands:
            if cmd[0] == 'vary':
                if nframes > cmd[2] and nframes < cmd[3]:
                    knobs[y][cmd[1]] = (y-cmd[2])(cmd[5]-cmd[4])/(cmd[3]-cmd[2])
